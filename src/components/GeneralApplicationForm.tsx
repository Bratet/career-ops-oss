...
export function GeneralApplicationForm({ onCreated, onBusy }: { onCreated?: () => void; onBusy?: (busy: boolean) => void }) {
  ...
  return (
    <form className="space-y-4 p-5" onSubmit={async (event) => {
      ...
      <p className="text-xs leading-relaxed text-[var(--color-muted)]">Choose a general resume and enter what you know. We’ll create a tracked application with its own copy, ready to tweak and finalize.</p>
      <fieldset disabled={busy || !!created} className="space-y-2">
        <legend className="text-xs font-medium text-[var(--color-text)]">Choose your general resume</legend>
        <div className="grid gap-2 sm:grid-cols-2">
          {([['en', 'English'], ['fr', 'French']] as const).map(([value, label]) => (
            <label key={value} className="flex cursor-pointer items-center gap-2 rounded-md border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-3 text-sm text-[var(--color-text)] has-[:checked]:border-[var(--color-accent)] has-[:checked]:bg-[var(--color-surface-2)] focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-[var(--color-accent)]">
              <input type="radio" name="language" value={value} checked={language === value} onChange={() => setLanguage(value)} className="accent-[var(--color-accent)]" />
              General resume in {label}
            </label>
          ))}
        </div>
      </fieldset>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button disabled={busy || !!created} className="flex items-center gap-2 rounded-md bg-[var(--color-accent)] px-3 py-2 text-xs font-medium text-[var(--color-bg)] disabled:opacity-50">{busy ? <Spinner /> : null}{busy ? 'Creating…' : `Create with ${language === 'en' ? 'English' : 'French'} resume`}</button>
      </div>
    </form>
  )
}
